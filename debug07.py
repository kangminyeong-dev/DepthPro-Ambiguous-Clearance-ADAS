...
# 경로 설정 (로컬용)
# ======================================================
img_paths  = "test_frames1"      # 입력 이미지 폴더  
result_dir = "result_frames1-2-1"    # 결과 이미지 폴더
...
# ======================================================
DEFAULT_MONODEPTH_CONFIG_DICT.checkpoint_uri = "ml-depth-pro/checkpoints/depth_pro.pt"
...
SAM2_CONFIG_DIR = os.path.join(os.getcwd(), "sam2", "sam2", "configs")
...
model_sam = build_sam2(model_cfg, checkpoint, device="cpu")
model_sam.to(device)
...
# ======================================================
# YOLOv8 Object Detector (2D Semantic Perception)
# ======================================================
...
for img_file in file_list:
    ...
    # -------------------------------------------------------
    # YOLOv8 Inference (2D Object Detection)
    # -------------------------------------------------------
    yolo_results = yolo_model(orig_rgb, verbose=False)[0]
    ...
    for box in yolo_results.boxes:
        ...
        if cls_id not in YOLO_CLASSES or conf < 0.4:
            continue
    ...
    if len(non_ground_pts) > 0:
        ...
        labels = db.labels_
        # -------------------------------------------------------
        # Build 2D bounding boxes for each 3D cluster
        # -------------------------------------------------------
        cluster_boxes = {}
        ...
        for lbl in set(labels):
            ...
            cluster_boxes[lbl] = (
                int(np.min(u_s_non_ground[idx])),
                int(np.min(v_s_non_ground[idx])),
                int(np.max(u_s_non_ground[idx])),
                int(np.max(v_s_non_ground[idx]))
            )
        ...
        # -------------------------------------------------------
        # Assign YOLO semantic class to each 3D cluster
        # -------------------------------------------------------
        cluster_class = {}

        for lbl, cbox in cluster_boxes.items():
            best_iou = 0.0
            best_class = "unknown"

            for det in detections:
                iou_val = iou(cbox, det["bbox"])
                if iou_val > best_iou:
                    best_iou = iou_val
                    best_class = det["class"]

            cluster_class[lbl] = best_class

        print("[YOLO–3D Fusion] Cluster classes:", cluster_class)
    ...
    # -------------------------------------------------------
    # Final ADAS Visualization (Merged: original + semantic coloring)
    # -------------------------------------------------------
    if final_pair:
        ...
        # -------------------------------------------------------
        # Semantic coloring map
        # -------------------------------------------------------
        cluster_colors = {}
        for lbl in set(labels):
            if lbl == -1:
                continue
            obj_type = cluster_class.get(lbl, "unknown")
            cluster_colors[lbl] = OBJECT_COLORS.get(obj_type, (180, 180, 180))

        # -------------------------------------------------------
        # Find nearest 3D points for top SAM overlay
        # -------------------------------------------------------
        ...
        # -------------------------------------------------------
        # Draw distance text and ROI polygon 
        # -------------------------------------------------------
        mid_u, mid_v = (uL + uR) // 2, (vL + vR) // 2
        cv2.putText(orig_img, f"{min_dist_fp:.2f} m",
                    (mid_u, mid_v - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 4, cv2.LINE_AA)
        cv2.putText(orig_img, f"{min_dist_fp:.2f} m",
                    (mid_u, mid_v - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, base_color, 2, cv2.LINE_AA)

        # ROI polygon with blended alpha
        ...
        AB = np.array([uR - uL, vR - vL], dtype=np.float32)
        a, b = AB[1], -AB[0]
        c_line = -(a * uL + b * vL)
        ...
        # -------------------------------------------------------
        # Prepare SAM overlay points (top + corners/midpoints)
        # -------------------------------------------------------
        ...
        uL_c, vL_c = int(u_s_non_ground[np.argmin(np.linalg.norm(non_ground_pts - centroid_left_3d, axis=1))]), \
                    int(v_s_non_ground[np.argmin(np.linalg.norm(non_ground_pts - centroid_left_3d, axis=1))])
        uR_c, vR_c = int(u_s_non_ground[np.argmin(np.linalg.norm(non_ground_pts - centroid_right_3d, axis=1))]), \
                    int(v_s_non_ground[np.argmin(np.linalg.norm(non_ground_pts - centroid_right_3d, axis=1))])

        # Top points slightly above centroid
        ...
        uL_plus, vL_plus = int(u_s_non_ground[np.argmin(np.linalg.norm(non_ground_pts - centroid_left_3d_plus, axis=1))]), \
                        int(v_s_non_ground[np.argmin(np.linalg.norm(non_ground_pts - centroid_left_3d_plus, axis=1))])
        uR_plus, vR_plus = int(u_s_non_ground[np.argmin(np.linalg.norm(non_ground_pts - centroid_right_3d_plus, axis=1))]), \
                        int(v_s_non_ground[np.argmin(np.linalg.norm(non_ground_pts - centroid_right_3d_plus, axis=1))])

        sam_points = [
            ([uL_plus, vL_plus], left_label2, "Left-Top"),
            ([uL_c, vL_c], left_label2, "Left-Bottom"),
            ([uR_plus, vR_plus], right_label2, "Right-Top"),
            ([uR_c, vR_c], right_label2, "Right-Bottom")
        ]

        # Add corner/midpoints
        for lbl, centroid_3d, centroid_2d in [(left_label2, centroid_left_3d, centroid_left),
                                            (right_label2, centroid_right_3d, centroid_right)]:
            poly = polygons[lbl]
            x_coords, z_coords = poly.exterior.xy
            corners = np.array([
                [np.min(x_coords), np.min(z_coords)],
                [np.min(x_coords), np.max(z_coords)],
                [np.max(x_coords), np.min(z_coords)],
                [np.max(x_coords), np.max(z_coords)]
            ])
            for cx, cz in corners:
                mid_3d = np.array([(cx + centroid_2d[0]) / 2, centroid_3d[1], (cz + centroid_2d[1]) / 2])
                ...
                sam_points.append(([u_mid, v_mid], lbl, "Corner-Mid"))

        # -------------------------------------------------------
        # Draw SAM overlays (semantic colors)
        # -------------------------------------------------------
        overlay = orig_img.copy()
        predictor.set_image(orig_img)
        for (pt, lbl, label_name) in sam_points:
            ...
            color_mask[mask_pt > 0] = cluster_colors.get(lbl, (180, 180, 180))
