# ================================================
# 17. 전방 대표 3개 객체만 선택 + 관통 거리선 제거 (연산량 절감 및 유효성 확보)
# ================================================
#
# 목적:
#   전방에 존재하는 다수 객체 중 실제 의미 있는 거리 관계만 측정하기 위해,
#   (1) 차량 전방에서 가장 가까운 3개 다각형만 추출하고,
#   (2) 두 객체 사이에 제3의 객체가 존재해 관통되는 거리선은 제거하여,
#   연산량을 줄이면서도 유효한 최소거리만 표시한다.
#
# 배경:
#   모든 객체쌍의 거리를 계산하면 계산량이 급증하고,
#   실제로도 객체 사이에 다른 객체가 끼어 있는 경우
#   그 최소거리는 주행 안전 측면에서 무의미하다.
#   따라서 이번 단계는 ‘가까운 3개 다각형’ + ‘관통 거리선 제거’ 조합으로
#   결과의 신뢰성과 실시간성(연산량)을 동시에 확보하는 실험이다.
#
#
# 터미널 실행 명령어:
#   python 00_baseline-depthpro-dev/17_select_top3_and_remove_crosslines.py
# ================================================
...
for fname in sorted(os.listdir(input_dir)):
    ...
    if len(polygons) == 0:
        print(f"{fname}: 다각형 없음, 스킵")
        continue

    # ------------------------------
    # 5단계: 전방 가장 가까운 3개 다각형만 선택
    # ------------------------------
    poly_with_dist = []
    for label, poly in polygons.items():
        ...
        poly_with_dist.append((label, poly, dist_from_origin))
    poly_with_dist.sort(key=lambda x: x[2])
    poly_with_dist = poly_with_dist[:3]
    polygons = {label: poly for label, poly, _ in poly_with_dist}

    # ------------------------------
    # 6단계: 최소 거리 계산 + 관통 거리선 제거
    # ------------------------------
    ...
    axes[1].set_title("Top-View Clustering & Min Distance (Top 3 & Crossline Removed)")
