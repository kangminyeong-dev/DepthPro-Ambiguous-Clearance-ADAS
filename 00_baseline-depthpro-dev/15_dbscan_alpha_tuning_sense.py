# ================================================
# 15. DBSCAN-AlphaShape 초기 파라미터 감각 확립 (eps·alpha 조정 및 노면 필터 보수화)
# ================================================
#
# 목적:
#   NuScenes Mini Dataset 일괄 적용 결과를 통해 실제 도로 표면의
#   경사나 굴곡으로 인한 오검출(노이즈)을 확인하고,
#   DBSCAN과 Alpha Shape의 초기 파라미터 감각을 확립한다.
#
# 배경:
#   실제 주행 도로는 완전히 평평하지 않다.
#   따라서 기존 z축 필터(-0.75~3.0m)는 지나치게 완화되어 있었고,
#   도로 굴곡·경사로 인해 지면 근처 잡음이 포함되는 현상이 관찰되었다.
#   이에 따라 z 범위를 좀 더 보수적으로 (-0.55~3.0m)로 조정하여
#   노면 필터링 효과를 강화했다.
#   또한 이 튜닝 과정에서 포인트를 탑뷰로 확인하는 것이 용이하였다.
#
# 파라미터 조정 이유:
#   - 감지 너비 -10 ~ 10(m) -> -5 ~ 5(m)
#     → 실제 주행 상을 고려하여, 내 중심 좌,우 한 차선에 조금만 더해서 보기로 설정했다.   
#   
#   - eps: 0.15 → 0.25  
#     → 하나의 객체(차량·벽체)가 두 클러스터로 분리되는 현상 완화  
#     → DBSCAN의 밀도 연결 기준을 완화하여 더 일관된 객체 그룹화 유도
#
#   - alpha: 0.05 → 0.10  
#     → Alpha Shape 다각형이 과도하게 각지지 않도록 완화  
#     → 외곽선의 안정성을 높여 실제 물체 형태에 더 근접한 폴리곤 생성
#
# 목표:
...
#
# 터미널 실행 명령어:
#   python 00_baseline-depthpro-dev/15_dbscan_alpha_tuning_sense.py
# ================================================
...
for fname in sorted(os.listdir(input_dir)):
    ...
    db = DBSCAN(eps=0.25, min_samples=25).fit(points[:,:2])
    labels = db.labels_
    ...
    for label in unique_labels:
        ...
        if len(cluster_points) < 100:
            continue
        shape = alphashape.alphashape(cluster_points, alpha=0.1)
